#include <bits/stdc++.h>

using namespace std;

#define SHIFTL(x, y) ((x) << (y))
#define SHIFTR(x, y) ((x) >> (y))

#define BIT(x) (1LL << (x))

typedef long long ll;
typedef unsigned long long ull;
typedef tuple<int, int> i2;
typedef tuple<int, int, int> i3;
typedef tuple<int, int, int, int> i4;
typedef vector<int> vi;
typedef vector<i2> vi2;
typedef vector<i3> vi3;
typedef vector<vi> v2i;
typedef vector<vi2> v2i2;
typedef vector<string> vs;
typedef vector<ll> vll;

int dx[] = {0, 1, 0, -1};
int dy[] = {1, 0, -1, 0};

bool isNeigh(int x1, int y1, int x2, int y2) {
    return abs(x1 - x2) + abs(y1 - y2) <= 1;
}

bool hasPassed(int origin, int dest, int obstacle) {
    if (dest > origin && dest >= obstacle && obstacle > origin) return true;
    if (dest < origin && dest <= obstacle && obstacle < origin) return true;
    return false;
}

bool isValid(int x1, int y1, int x2, int y2) {
    return x1 >= 0 && x1 < 8 && y1 >= 0 && y1 < 8 && !isNeigh(x1, y1, x2, y2);
}

int main() {
    ios_base::sync_with_stdio(false); cin.tie(nullptr);
    int n, m;

    while (cin >> n >> m && n | m) {
        int x = n, y = m;
        if (x > y) swap(x, y);
        if (x >= 3 && y >= 3) {
            cout << (int) ceil(x * y / 2.0) << " knights may be placed on a " << n << " row " << m << " column board." << endl;
        } else if (x == 2 && y >= 2) {
            cout << 4 * (y / 4) + 2 * min(2, y % 4) << " knights may be placed on a " << n << " row " << m << " column board." << endl;
        } else if (x == 1) {
            cout << y << " knights may be placed on a " << n << " row " << m << " column board." << endl;
        } else cout << 0 << " knights may be placed on a " << n << " row " << m << " column board." << endl; 
    }

    return 0;
}